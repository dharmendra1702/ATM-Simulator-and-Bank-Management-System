...
public class ws extends JFrame {
    ...
    ws(String pinnumber) {
        ...
        try {
            ...
            // Fetch latest withdrawal transaction
            ResultSet rsWithdraw = conn.s.executeQuery(
                "SELECT * FROM bank WHERE pinnumber = '" + pinnumber + "' AND type = 'Withdraw' ORDER BY date DESC LIMIT 1"
            );

            String latestDate = null, latestAmount = null;
            if (rsWithdraw.next()) {
                latestDate = rsWithdraw.getString("date");
                latestAmount = rsWithdraw.getString("amount");
            }
            ...
            // Build receipt
            mini.append("       XYZ BANK WITHDRAW RECEIPT\n");
            mini.append("=====================================\n");
            ...
            if (latestAmount != null) {
                ...
                mini.append("Transaction Type: Withdraw\n");
                mini.append("Amount Withdrawn: Rs " + latestAmount + "\n");
            } else {
                mini.append("No recent withdrawal found.\n");
            }
    ...
    public static void main(String[] args) {
        new ws(""); // test manually with valid pin number in DB
    }
}
