...
public class fs extends JFrame {
    ...
    fs(String pinnumber) {
        ...
        try {
            ...
            // Fetch latest Withdrawl transaction (used for Fast Cash)
            ResultSet rsFastCash = conn.s.executeQuery(
                "SELECT * FROM bank WHERE pinnumber = '" + pinnumber + "' AND type = 'Withdrawl' ORDER BY date DESC LIMIT 1"
            );

            String latestDate = null, latestAmount = null;
            if (rsFastCash.next()) {
                latestDate = rsFastCash.getString("date");
                latestAmount = rsFastCash.getString("amount");
            }
            ...
            // Build receipt
            mini.append("       XYZ BANK FAST CASH RECEIPT\n");
            mini.append("=====================================\n");
            ...
            if (latestAmount != null) {
                ...
                mini.append("Transaction Type: Withdrawl\n");
                mini.append("Amount Withdrawn: Rs " + latestAmount + "\n");
            } else {
                mini.append("No recent Fast Cash transaction found.\n");
            }
        ...
        // --- Return to transactions page when closed ---
    ...
    public static void main(String[] args) {
        new fs(""); // test manually with valid pin number
    }
}
